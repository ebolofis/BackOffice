...
/**
 * @ngdoc function
 * @name posBOApp.controller:TableReservationCtrl
 * @description
...
angular.module('posBOApp')
    .controller('TableReservationCtrl', [
        'tosterFactory', '$stateParams', '$scope', '$rootScope', '$q', '$interval', '$uibModal', '$mdDialog', '$mdMedia', 'DynamicApiService', 'dataUtilFactory', 'TableResService', 'trFactory',
        function (tosterFactory, $stateParams, $scope, $rootScope, $q, $interval, $uibModal, $mdDialog, $mdMedia, DynamicApiService, dataUtilFactory, TableResService, trFactory) {
            ...
            $scope.initView = function () {
                $scope.viewTitle = TableResService.getcontainerTitle($scope.editingEntity);
                ...
                //if ($scope.containerTemplate == null) $scope.containerTemplate = 'error-container-template';
            }
            ...
            $scope.apiActions = {
                'GetReservationTypes': function () {
                    var url = trFactory.apiInterface.ReservationTypes.GET.GetReservationTypes;
                    return DynamicApiService.getV3('ReservationTypes', url)
                        .then(function (result) { return result; }).catch(function (rejection) { $scope.exHandler(rejection, 'GetReservationTypes'); });
                },
    ...
    ]).service('TableResService', ['$http', 'config', 'tosterFactory', 'trFactory', function ($http, config, tosterFactory, trFactory) {
        var templates = {
        };
        ...
        var titles = {
            ...
            'RestrictionAssocs': 'Restaurant Restriction Associations',
            'TradingHours': 'Trading Hours',
            'ExcludedRestrictions': 'Restaurant Excluded Restrictions',

            'Capacities': 'Restaurant Capacities',
            'OverwrittenCapacities': 'Restaurant Overwritten Capacities',
            'ExcludedDays': 'Capacities Excluded Days',
            'Reservations': 'Restaurant Reservations',
            'Customers': 'Restaurant Customers',

            'errorLoad': 'Invalid Table Reservation Module'
        };

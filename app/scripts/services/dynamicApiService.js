...
angular.module('posBOApp')
    .service('DynamicApiService', ['$http', 'config', 'auth', '$base64', function ($http, config, auth, $base64) {
        ...
        this.getPayroll = function (RoutePrefix, Route, dataObj) {
            var authSpecs = auth.getLoggedSpecs();
            $http.defaults.headers.common['Authorization'] = authSpecs.auth;
            var d = (dataObj != null) ? '/' + dataObj : '';
            var urlResource = urlBase  + RoutePrefix + '/' + Route;
            console.log(urlResource)
            return $http.get(urlResource + d);
        }
        this.getPromoDetails = function (RoutePrefix, Route, dataObj) {
            var authSpecs = auth.getLoggedSpecs();
            $http.defaults.headers.common['Authorization'] = authSpecs.auth;
            var d = (dataObj != null) ? '/' + dataObj : '';
            var urlResource = urlBase + 'v3' + '/' + RoutePrefix + '/' + Route ;
        ...
        this.getV3promos = function (RoutePrefix, Route) {
            var authSpecs = auth.getLoggedSpecs();
            $http.defaults.headers.common['Authorization'] = authSpecs.auth;
            var d = (dataObj != null) ? '/' + dataObj : '';
            var urlResource = urlBase + 'v3' + '/promos' + RoutePrefix + '/' + Route;
            console.log(urlResource)
            return $http.get(urlResource + d);
        }
        this.getV3stores = function (RoutePrefix, Route) {
            var sUsername = localStorage.getItem("staffUserName");
        ...
        this.postV3CustomerDataConfig = function (RoutePrefix, Route, dataObj) {
            debugger
            var authSpecs = auth.getLoggedSpecs();
            $http.defaults.headers.common['Authorization'] = authSpecs.auth;
        ...
        this.postV3Stores = function (RoutePrefix, Route, dataObj) {
            var sUsername = localStorage.getItem("staffUserName");
            var staffPassword = localStorage.getItem("staffPassword");
            var staffUsername = "";
            if (sUsername.endsWith("@")) {
            ...
            var authNew = $base64.encode("" + staffUsername + ":" + staffPassword + "");
            $http.defaults.headers.common['Authorization'] = 'Basic ' + authNew;
            var urlResource = localStorage.getItem("baseURL") + 'api/' + 'v3' + '/da/' + RoutePrefix + '/' + Route;
            return $http({ url: urlResource, method: "POST", dataType: 'JSON', data: JSON.stringify(dataObj), });
        }
        this.postGetPmsDepartments = function (RoutePrefix, Route, dataObj) {
            var authSpecs = auth.getLoggedSpecs();
            $http.defaults.headers.common['Authorization'] = authSpecs.auth;
            var urlResource = urlBase + 'v3' + '/' + RoutePrefix + '/' + Route;
            return $http.post(urlResource, dataObj);
        }
        this.postGetTransferMappings = function (RoutePrefix, Route, dataObj) {
            var authSpecs = auth.getLoggedSpecs();
            $http.defaults.headers.common['Authorization'] = authSpecs.auth;
            var urlResource = urlBase + 'v3' + '/' + RoutePrefix + '/' + Route;
            return $http.post(urlResource, dataObj);
        ...
        this.postProdCatVatChanges = function (RoutePrefix, Route, dataObj) {
            var authSpecs = auth.getLoggedSpecs();
            $http.defaults.headers.common['Authorization'] = authSpecs.auth;
            var urlResource = urlBase + 'v3' + '/' + RoutePrefix + '/' + Route;
            return $http({ url: urlResource, method: "POST", dataType: 'JSON', data: JSON.stringify(dataObj), });
        ...
        this.DeleteHeader = function (RoutePrefix, Route, Id) {
            var authSpecs = auth.getLoggedSpecs();
            $http.defaults.headers.common['Authorization'] = authSpecs.auth;
            var urlResource = urlBase + 'v3' + '/' + RoutePrefix + '/' + Route + Id;
            return $http({ url: urlResource, method: "POST", dataType: 'JSON' });
        }
        this.DeleteStore = function (RoutePrefix, Route, Id) {
            var sUsername = localStorage.getItem("staffUserName");
            var staffPassword = localStorage.getItem("staffPassword");
            var staffUsername = "";
            if (sUsername.endsWith("@")) {
            ...
            $http.defaults.headers.common['Authorization'] = 'Basic ' + authNew;
           // var urlResource = localStorage.getItem("baseURL") + 'api/' + 'v3' + '/da/' + RoutePrefix + '/' + Route;
            var urlResource = urlBase + 'v3' + '/da/' + RoutePrefix + '/' + Route  + Id ;
            return $http({ url: urlResource, method: "POST", dataType: 'JSON' });
        }
        ...
        this.updateStoreTables = function (RoutePrefix, Route, dataObj) {
            var sUsername = localStorage.getItem("staffUserName");
            var staffPassword = localStorage.getItem("staffPassword");
            var staffUsername = "";
            if (sUsername.endsWith("@")) {
            ...
            var urlResource = localStorage.getItem("baseURL") + 'api/' + 'v3' + '/da/' + RoutePrefix + '/' + Route;
            console.log(urlResource)
            //return $http({ url: urlResource, method: "GET", dataType: 'JSON', data: JSON.stringify(dataObj), });
            return $http.get(urlResource + dataObj);
        }
        this.getPolygonsIsActiveV3 = function (RoutePrefix, Route) {
            var sUsername = localStorage.getItem("staffUserName");
            var staffPassword = localStorage.getItem("staffPassword");
            var staffUsername = "";
            if (sUsername.endsWith("@")) {
        ...
        this.insertShortage = function (RoutePrefix, Route, dataObj) {
            var sUsername = localStorage.getItem("staffUserName");
            var staffPassword = localStorage.getItem("staffPassword");
            var staffUsername = "";
            if (sUsername.endsWith("@")) {
        ...
        this.deleteShortageDAV3 = function (RoutePrefix, Route, dataObj) {
            var sUsername = localStorage.getItem("staffUserName");
            var staffPassword = localStorage.getItem("staffPassword");
            var staffUsername = "";
            if (sUsername.endsWith("@")) {
            ...
            var authNew = $base64.encode("" + staffUsername + ":" + staffPassword + "");
            $http.defaults.headers.common['Authorization'] = 'Basic ' + authNew;
            var urlResource = localStorage.getItem("baseURL") + 'api/' + 'v3' + '/da/' + RoutePrefix + '/' + Route + '/' + dataObj ;
            return $http({ url: urlResource, method: "POST", dataType: 'JSON' });
        }
        ...
        this.getDynamicObjectDataPayroll = function (apiControllerName, parameters) {
            var authSpecs = auth.getLoggedSpecs();
            $http.defaults.headers.common['Authorization'] = authSpecs.auth;
            //parameters = '?' + parameters;
            //return $http.get(urlBase + apiControllerName + parameters);
            ...
            var sid = '?storeId=' + authSpecs.storeId;
            console.log(urlBase + apiControllerName + sid );
            return $http.get(urlBase + apiControllerName + sid );
        }


        this.getDynamicObjectDataStores = function (apiControllerName, parameters) {

            if (localStorage.getItem("isDa") == true) {
                var authSpecs = auth.getLoggedSpecs();
                $http.defaults.headers.common['Authorization'] = authSpecs.auth;
                //parameters = '?' + parameters;
                //return $http.get(urlBase + apiControllerName + parameters);
                ...
                var sid = '?storeId=' + authSpecs.storeId;
                console.log(urlBase + apiControllerName + parameters);
                return $http.get(urlBase + 'v3/da/Stores/' + apiControllerName + '/GetBOStores');
            }
            else
                return true;
        }
        this.getDynamicObjectDataDA = function (apiControllerName, apiControllerFunction, parameters) {
            var DAAuth = "Basic " + $base64.encode(localStorage.staffUserName + ':' + localStorage.staffPassword);
            $http.defaults.headers.common['Authorization'] = DAAuth;
            //parameters = '?' + parameters;
            //return $http.get(urlBase + apiControllerName + parameters);
            if (parameters !== undefined && parameters !== null && parameters !== '')
                parameters = '&' + parameters;
            else parameters = '';
            console.log(localStorage.getItem("baseURL") + 'api/' + 'v3/da/'  + apiControllerName + '/' + apiControllerFunction);
            return $http.get(localStorage.getItem("baseURL") + 'api/' + 'v3/da/' + apiControllerName + '/' + apiControllerFunction);
        }
